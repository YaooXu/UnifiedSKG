...
class T5Attention(nn.Module):
    def __init__(self, config: T5Config, has_relative_attention_bias=False):
        ...
        # TODO: Xu
        self.offset = 64
        self.encoder_position_bias = nn.Embedding(128, self.n_heads) 
        # initialize is important
        self.encoder_position_bias.weight.data.normal_(mean=0.0, std=config.initializer_factor * ((config.d_model) ** -0.5))
    ...
    def forward(
        ...
        if not self.is_decoder:
            # TODO: Xu, only add relative position in encoder in each layer
            position_bias = self.encoder_position_bias(position_bias + self.offset).permute(0, 3, 1, 2)
        else:
            if position_bias is None:
                if not self.has_relative_attention_bias: # only first layer in Enc, it is true
...
class T5Stack(T5PreTrainedModel):
    ...
    def forward(
        ...
        # position_bias = None
        position_bias = encoder_position_bias # TODO: Xu
        encoder_decoder_position_bias = None
        ...
        for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
            ...
            if self.is_decoder:
                position_bias = layer_outputs[2] # TODO: Xu, only share the position biases in decoder
    ...
    def forward(
        ...
        if encoder_outputs is None:
            ...
            encoder_outputs = self.encoder(
                ...
                output_hidden_states=output_hidden_states,
                return_dict=return_dict,
                encoder_position_bias=encoder_position_bias # TODO: Xu
            )
        elif return_dict and not isinstance(encoder_outputs, BaseModelOutput):
            encoder_outputs = BaseModelOutput(
                last_hidden_state=encoder_outputs[0],
        ...
        # TODO: Xu, from 2-d attn_mask (used in encoder) to 1-d attn_mask (used in cross-attention)
