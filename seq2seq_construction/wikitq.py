...
def _process_table(process_idx, raw_datasets, tab_processor, is_train=True):
    ...
    for i, raw_data in enumerate(raw_datasets):
        ...
        extend_data['graph'] = tab_processor.table_linearize_func.get_position_bias_and_attn_mask(linear_table.lower())
    ...
    def __init__(self, args, raw_datasets, cache_root):
        ...
        if os.path.exists(cache_path) and args.dataset.use_cache:
            ...
            for expand_id in range(expansion):
                self.extended_data.extend(
                    parallel_process_tables(self.raw_datasets, self.tab_processor, True)
                )
...
class DevDataset(Dataset):
    ...
    def __init__(self, args, raw_datasets, cache_root):
        ...
        if os.path.exists(cache_path) and args.dataset.use_cache:
            ...
            self.extended_data = (
                    parallel_process_tables(self.raw_datasets, self.tab_processor, False)
            )
            # for raw_data in tqdm(self.raw_datasets):
            #     extend_data = deepcopy(raw_data)
            #     question = extend_data["question"]
